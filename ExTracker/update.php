<?php
  ...
  $user1 = new User();
  $user1->generate(Input::get('id_user'));
  $akun = new Akun();
  $akun->generate(Input::get('id_akun'));

  $emailAwal = $user1->getItem('email');
  $usernameAwal = $akun->getItem('username');

  if (!empty($_POST)) { // form submitted

    $error_message1 = $user1->validate($_POST);
    ...
    if(!empty($error_message1)) {
      ...
      if($error_message1['email'] == "Email sudah terdaftar"){
        ...
        if($emailAwal == $user1->getItem('email')){
    ...
    if (empty($error_message1) && empty($error_message2)) {
      $user1->update(Input::get('id_user'));
      $akun->update(Input::get('id_akun'));
      $message = urlencode("User dengan nama <b>{$user1->getItem('nama')}</b> sudah berhasil di-update");
  ...
  <section>
    ...
    <form action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post" class="form">
      <div class="mb-3">
        ...
        <input type="text" style="border: 2px solid #009d63;"  name="nama" id="nama" class="form-control" value="<?php echo $user1->getItem('nama'); ?>">
      </div>
      <div class="mb-3">
      ...
      <div class="mb-3">
        ...
        <input type="text" style="border: 2px solid #009d63;"  name="no_telepon" id="no_telepon" class="form-control" value="<?php echo $user1->getItem('no_telepon'); ?>">
      </div>
      <div class="mb-3">
        <label for="email" class="form-label">Email</label>
        <input type="email" style="border: 2px solid #009d63;"  name="email" id="email" class="form-control"
          value="<?php echo $user1->getItem('email'); ?>">
      </div>
      <br>
