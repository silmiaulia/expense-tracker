<?php
  ...
  $minimum_saldo = new Minimum_Saldo();

  
  $check = $minimum_saldo->generate($id_akun);

    if(!empty($minimum_saldo->getItem('jumlah_minimum_saldo'))){
        $isUpdate = 1;
  ...
  if (!empty($_POST)) { // form submitted
    ...
    $error_message = $minimum_saldo->validate($_POST);

    if (empty($error_message)) {
        
        if($isUpdate == 1){
            $minimum_saldo->update($id_akun);
            $message = urlencode("Success Update Minimum Saldo");
        }else{
            $minimum_saldo->insert();
            $message = urlencode("Success Add Minimum Saldo");      
        }
        ...
        <div id="page-content-wrapper">
            <nav class="navbar navbar-expand-lg navbar-light bg-transparent py-4 px-4">
                <div class="d-flex align-items-center">
                    ...
                    <h2 class="fs-2 m-0">Add Minimum Saldo</h2>
                </div>
            ...
            <div class="container-fluid px-4">
                <div class="container my-5 pt-4 pb-5 px-5 rounded bg-white" style="max-width:500px">
                    <h1 class="text-center mb-4">Add Minimum Saldo</h1>
                    <?php
                    ...
                    <input type="hidden" id="id_akun" name="id_akun" value="<?php echo $id_akun; ?>">
                        <div class="mb-3"">
                            <div class="input-group input-group-merge">
                                ...
                                class="form-control"
                                placeholder="1000"
                                name="jumlah_minimum_saldo"
                                id="jumlah_minimum_saldo"
                                value="<?php echo $minimum_saldo->getItem('jumlah_minimum_saldo'); ?>"
                                />
                        ...
                        <?php
                            ...
                            <input style="border: 2px solid #009d63; background-color:#009d63;" type="submit" name="submit" value="<?php echo $value_btn; ?>" class="w-100 btn btn-primary">
